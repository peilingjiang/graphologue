  ...
  ({
    ...
    const handleSelfCorrection = useCallback(
      async (answerObject: AnswerObject): Promise<string> => {
        ...
        if (jobs.length === 0) return originTextContent
        ...
        const replacements = await Promise.all(
          jobs.map(async ({ sentence, n, e }) => {
            ...
            const replacement = ' ' + correctionText
        ...
        return applyTextReplacements(originTextContent, replacements)
      },
      [question],
    )
        ...
        newAnswerObject[target].edgeEntities = newAnswerObject[
          ...
          .map((e: EdgeEntity) => ({
            ...
            edgePairs: e.edgePairs.filter(
              eP =>
                eP.sourceId !== nodeEntityId && eP.targetId !== nodeEntityId,
            ),
          }))
    ...
    const handleAnswerObjectNodeCollapse = useCallback(
      (answerObjectId: string, nodeEntityId: string) => {
        ...
        if (
          answerObject.answerObjectSynced.collapsedNodes.includes(nodeEntityId)
        )
          // remove
